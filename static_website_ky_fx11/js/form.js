$(function() {
    ...
    function get_query_array(query) {
        ...
        if (qPos !== -1 || query.indexOf("=") !== -1) {
            for (; i >= 0; i--) {
                ...
                obj[unescape(s[0])] = s.hasOwnProperty(1) ? unescape(decodeURIComponent(s[1])) : null;
            };
        }
        return obj;
    }
    ...
    function refresh() {
        if (flag) {
            $('#buy').removeAttr('disabled').removeClass('btn-disabled');
        } else {
            $('#buy').addClass('btn-disabled');
        }
    }
    ...
    $('#terms').click(function () {
        // 1000 * 550 
        $.modal('<iframe src="./terms.html" width="877" height="458" style="border:0">', {
            dataId: 'termsbox',
            overlayClose: true,
            minWidth: 877,
            minHeight: 458
    ...
    $('#conditions').click(function () {
        $.modal('<iframe src="./conditions.html" width="877" height="458" style="border:0">', {
            dataId: 'termsbox',
            overlayClose: true,
            minWidth: 877,
            minHeight: 458
    ...
    $('#buy').click(function () {
        if (!flag) {
            $.modal("<div class='tipMsg'>请勾选阅读《投保须知》和《保险条款》</div>", default_settings);
            return false
        }

        if ($('#form').isValid(lang, conf, false)) {
            if (!ids[0] || !ids[1]) {
                return $.modal("<div class='tipMsg'>渠道号或设备号缺失</div>", default_settings);
            }
            ...
            $.ajax('//m.kaikaibao.com.cn/api/event/zijia', {
                ...
                if (!res.success) {
                    $.modal("<div class='tipMsg'>提交失败</div>", default_settings);
                } else {
                    location.href = './pay.html#/changjing/' + location.hash.split('/').slice(-2).join('/');
                }
            }).fail(function (res, textStatus, errorThrown) {
                if (res.status == 422) {
                    $.modal("<div class='tipMsg'>请检查所有的输入项，身份证号码错误</div>", default_settings);
                } else {

                }
            }).catch(function () {
                // console.log(arguments);
                $.modal("<div class='tipMsg'>订单提交失败，请稍后再试。</div>", default_settings);
            })
        } else {
            var msg = '';
            if (errors.length >= 3) {
            ...
            errors = []
            $.modal("<div class='tipMsg'>" + msg + "</div>", default_settings);
        }
    })
});