$(function() {
    sessionStorage.setItem('form-url',location.href)
    $('#licenseno').val(sessionStorage.getItem('form-licenseno'))
    $('#idcard').val(sessionStorage.getItem('form-idcard'))
    $('#name').val(sessionStorage.getItem('form-name'))
    $('#mobile').val(sessionStorage.getItem('form-mobile'))
    ...
    $('.form input').click(function(e){
        location.href='input.html?type='+e.target.id
    })
    ...
    $('#terms').click(function () {
        $.modal('<iframe src="./terms.html" height="500" width="940" style="border:0">', {
            dataId: 'termsbox',
            minWidth: 960,
            minHeight: 500,
            overlayClose: true
    ...
    $('#conditions').click(function () {
        $.modal('<iframe src="./conditions.html" height="500" width="940" style="border:0">', {
            dataId: 'termsbox',
            overlayClose: true,
            minWidth: 960,
            minHeight: 500,
    ...
    $('.buy').click(function () {
        if (!flag) {
            $.modal("<div><h1>请阅读《投保须知》 《保险条款》</h1></div>", default_settings);
            return false
        }
        ...
        if ($("#form").isValid(lang, conf, false)) {
            $.ajax('//m.kaikaibao.com.cn/api/event/zijia', {
                ...
                if (res.status == 422) {
                    $.modal("<div><h1>请填写正确的身份证号</h1></div>", default_settings);
                } else {
